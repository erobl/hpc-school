...
int main(int argc, char** argv){
	...
	MPI_Comm_create_group(MPI_COMM_WORLD, working_group, TAG, &working_comm);
	...
	for(i=1; i<=iterations; i++){
		...
		MPI_Send(locals, number*13, MPI_DOUBLE, next, TAG, MPI_COMM_WORLD);
		MPI_Recv(foreigners, number*13, MPI_DOUBLE, previous, TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		evolve(locals, foreigners, number, foreignNumber);
		//TO DO: running the algorithm for (p-1)/2 rounds. REMEMBER: call evolve function
		int iter;
		for(iter = 0; iter < (p-1)/2 - 1; iter++) {
			MPI_Send(foreigners, number*13, MPI_DOUBLE, next, TAG, MPI_COMM_WORLD);
			MPI_Recv(foreigners, number*13, MPI_DOUBLE, previous, TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			evolve(locals, foreigners, number, foreignNumber);
		...
		MPI_Send(foreigners, number*13, MPI_DOUBLE, (myRank+p-iter-1) % p, TAG, MPI_COMM_WORLD);

		//TO DO: receiving the incoming particles and merging them with the local set, interacting the local set
		MPI_Recv(foreigners, number*13, MPI_DOUBLE, (myRank+iter+1) % p, TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
